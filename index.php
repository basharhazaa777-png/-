...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'admin_login') {
    if (isset($_POST['pin_code']) && $_POST['pin_code'] === $_SESSION['bashar_pin']) {
        ...
        // هنا السر: نبلغ الجافا سكريبت بالنجاح وتخزين الحالة محلياً فوراً لمنع الخروج
        echo "<script>localStorage.setItem('bashar_admin_auth', 'true'); window.location.href=window.location.pathname;</script>";
        exit;
    } else {
...
<body>
    ...
    <script>
        ...
        // فحص التحقق الثنائي (سيرفر + متصفح) لمنع الخروج العشوائي نهائياً بعد التحديث
        ...
        const App = {
            init: () => { 
                ...
                // فتح لوحة المسؤول فوراً وبثبات مطلق
